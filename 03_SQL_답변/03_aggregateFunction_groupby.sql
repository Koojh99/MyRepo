...
select distinct ifnull(dept_name,'a') from emp;  -- 부서가 몇개
select count(distinct ifnull(dept_name,'a')) from emp;   -- 결측치 포함한 갯수 
...
from	emp
group by dept_name 
having	avg(salary) >= 5000; -- and count(*) >= 5 alter，　미리　ｈａｖｉｎｇ으로　걸러낸　다음　ｓｅｌｅｃｔ
...
select	year(hire_date),         -- 쿼리 실행 순서 (5)
		avg(salary)
from	emp						-- (1)
