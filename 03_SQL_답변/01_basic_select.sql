...
select * from emp;     -- use hr 이므로 hr.emp 할 필요 없음 
...
select distinct job      -- distinct :범주형 컬럼에서 범주값이 어떻게 구성되었는지 확인.
from emp;

-- select distinct job, dept_name from emp; : distinct은 맨 앞에 한번만 사용 가능, (job,dept_name) 합쳐서 조회 한번만 뜨도록 

-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
select emp_id as 직원ID,    -- 조회할 컬럼 as 내가 나타낼 컬럼, as 생략가능, as 다음에는 한 단어로만 해야한다.
	   emp_name as 직원이름, -- 직원이름 -> 직원 이름(x), "직원 이름"(O)
       hire_date as 입사일,
       salary as 급여,
       dept_name as 소속부서
from emp;
...
select emp_id,
       ...
       salary,
       comm_pct,
       salary * comm_pct as "commission"     -- 같은 행에 있는 값끼리 산수 / 값 * null = null 
from emp;

-- 직원 이름(emp_name)과 salary를 조회. salary앞에 '$'를 붙여서 출력. 
select emp_name,
	   concat('$',salary) as salary      -- 값이므로 작은 따옴표 
from emp;
...
/* *************************************
where 절을 이용한 행 선택 , 특정 조건의 행만 가려서 (-> 행 선택 조건)
...
select emp_id,
       ...
       comm_pct
from emp
where comm_pct between 0.2 and 0.3;     -- between 0.2001 and 0.3001, 이러면 0.2와 0.3은 포함x
-- where comm_pct >= 0.2
-- and comm_pct <= 0.3;
...
select emp_id,
	   emp_name
from   emp
where  emp_name like '%!%%' escape '!';   -- !은 원래 기능 !이아니라는말, 단순히 구분 문자로 사용하겠다. ex \n은 '\'은 구분하기 위해 사용한것
...
order by dept_name;   -- default asc
...
order by 3 desc;     -- 내가 select한 컬럼중에 세번째 컬럼 기준으로 정렬 
