...
select	emp_id,
        ...
        ceil(salary * 1.15) - salary -- SAL_RAISE - salary 내가 지정한 별칭 값을 임의로 연산 불가능 
from	emp;
...
select	emp_id,
		emp_name,
        comm_pct,
        round(comm_pct * 1.08) as "comm	_raise",
        round(salary, -3)
from	emp
...
select	hire_date,
		...
		subdate(hire_date, interval 10 month) "입사 10개월 후 후"
from	emp
where	dept_name = 'IT';
...
***********************************************/
select '10' + '20';
select datediff(curdate(),'2025-03-20');   -- 뒤에 날짜에서 앞에 날짜까지 얼마나 차이가 나냐 
-- 시간을 정수형태로 변환   
select	now(),
		cast(now() as signed),   -- 부호있는 정수로 변환
		cast(now() as char);	-- 문자로 변환
...
select	concat('a',10,5.3,'aaaa');    -- 타입과 상관없이 다 붙여준다. 
...
select	salary,
		case when salary >= 10000 then '1등급'    -- case salary when >= 10000 라고 생각하지만, when 다음에 값이 나와야하므로
			else '2등급'
            end as "salary grade"
from emp;
...
select	emp_id,
        ...
        case when salary < 5000 then salary * 0.3 
			 when salary >= 10000 then salary * 0.1      -- when salary >= 10000 then salary * 0.1
             else salary * 0.2
             end as "Salary raise"
from emp;
